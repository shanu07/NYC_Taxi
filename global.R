...
vendor_id <-c(1,2)
week_day <- c(1:7)
# final = (fread("../proc_final.csv", sep = "auto"))
# compid = (fread("../comp_id.csv", sep = "auto"))
# set.seed(1234)
# grand <-sample_n(final,100000)
choice <- colnames(grand)[-1]
vid1 <-compid %>% filter(vendor_id == 1) 
vid2 <-compid %>% filter(vendor_id == 2) 

# save(final, file = "final.Rda")
load("final.Rda")
#save(grand, file = "grand.Rda")
load("grand.Rda")
